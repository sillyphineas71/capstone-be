...
- ❌ Chia sẻ TypeORM Entity giữa module (mỗi module có DTO/Entity riêng, không import Entity module khác trực tiếp)
...
async function outboxWorkerTick() {
  // [Needs DB compact mapping review] email_outbox table không thuộc DB Compact 39 tables;
  // Đây là infrastructure pattern cần mapping. Dùng dataSource.transaction + manager.query(),
  // không tạo TypeORM Entity.
  await dataSource.transaction(async (manager) => {
    ...
    await manager.query(
      `UPDATE email_outbox SET status = $1, attempts = $2 WHERE id = $3`,
      ['SENDING', row.attempts + 1, row.id]
    );
    ...
    try {
      ...
      await manager.query(
        ...
        ['SENT', new Date(), row.id]
      );
    } catch (e) {
      const isFinal = row.attempts + 1 >= row.max_attempts;
      const backoffMs = Math.min(2 ** row.attempts * 60_000, 30 * 60_000);
      await manager.query(
        `UPDATE email_outbox SET status = $1, last_error = $2, next_retry_at = $3 WHERE id = $4`,
        [isFinal ? 'DEAD' : 'RETRY', String(e), new Date(Date.now() + backoffMs), row.id]
      );  // exp backoff cap 30min
        }
      });
    }
  });
...
| BE #1 | Khởi tạo NestJS skeleton, TypeORM setup + DataSource config, migration đầu, base module structure | 16 |
...
| D-02 | NestJS + TypeORM + Postgres làm core [historical: rejected] | T1/W1 | Tech Lead | TS đầy đủ, module DI chuẩn, ORM mạnh | Học NestJS có cost ~1 tuần | Locked |
...
```sql
-- apps/backend/src/database/migrations/20260601_booking_exclude.sql
...
```typescript
// [Needs DB compact mapping review] email_outbox không thuộc DB Compact 39 tables;
// Dùng DataSource + manager.query() raw SQL thay PrismaService; không tạo TypeORM Entity.
// Khi có quyết định mapping, review dùng notifications table hay infrastructure queue.
    ...
    await this.dataSource.transaction(async (manager) => {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          await manager.query(
            `UPDATE email_outbox SET status = $1, last_error = $2, next_retry_at = $3 WHERE id = $4`,
            [isFinal ? 'DEAD' : 'RETRY', String(e?.message ?? e), new Date(Date.now() + delayMs), row.id]
          );
...
async validateRefreshToken(jti: string, userId: number): Promise<boolean> {
  // [Needs DB compact mapping review] refresh_tokens table KHÔNG tồn tại trong DB Compact 39 tables.
  // AGENTS.md: dùng Stateless JWT Blacklist qua Redis Cache + audit_logs tracking.
  // Không tạo Entity. Hướng mapping:
  //   - refresh_tokens -> Redis Cache (JWT blacklist + whitelist)
  //   - Nếu cần persistence -> audit_logs (action_type='TOKEN_ISSUED')
  //   - password_changed_at -> users.password_updated_at (DB Compact)
  const user = await this.dataSource.getRepository(UserEntity).findOneBy({ id: userId });
  if (!user) return false;
  // Check cache layer (Redis) thay vì DB table:
  // if (await this.redisService.sismember('token:blacklist', jti)) return false;
  // Check iat claim từ JWT vs password_updated_at:
  // const decoded = jwt.decode(tokenString) as { iat: number };
  // if (!decoded) return false;
  // if (user.password_updated_at && decoded.iat < Math.floor(user.password_updated_at.getTime() / 1000)) {
  //   return false;
  // }
  return true;
}
