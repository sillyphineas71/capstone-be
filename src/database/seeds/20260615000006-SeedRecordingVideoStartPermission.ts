...
/**
 * Seed permission cho REC-002 (Bắt đầu ghi hình video).
 * Bám convention seed hiện có. (seed-runner chưa wire — vấn đề team-wide.)
 */
export async function seedRecordingVideoStartPermission(
  ...
  try {
    const permissionResult = await queryRunner.query(
      ...
      [
        ...
        'Bắt đầu ghi hình từ IP camera',
        'recording',
        'video_start',
        'Cho phép bắt đầu ghi hình video (ffmpeg) từ IP camera cho cuộc họp.',
      ],
    );
