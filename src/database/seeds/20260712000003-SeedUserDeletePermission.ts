...
/**
 * Seed: permission `accounts.user.delete` (UC-10 — Xóa tài khoản người dùng,
 * DELETE /api/v1/users/:userId).
 *
 * Role-set: CHỈ `SYSTEM_ADMIN`. Đây là thao tác phá hủy (soft-delete) nhạy cảm,
 * không giao cho Business Admin.
 ...
 * sau khi được duyệt.
 */
export async function seedUserDeletePermission(
  ...
  try {
    const permResult = await queryRunner.query(
      'INSERT INTO permissions (permission_code, permission_name, module_code, action_code, description, is_active) ' +
        "VALUES ('accounts.user.delete', 'Xóa tài khoản người dùng', 'accounts', 'delete', 'Cho phép xóa (soft-delete) một tài khoản người dùng trong module accounts', true) " +
    ...
    if (!permissionId) {
      const existing = await queryRunner.query(
        "SELECT id FROM permissions WHERE permission_code = 'accounts.user.delete';",
      );
      if (existing[0]?.id) {
...
async function assignPermissionToRoles(
  ...
  // CHỈ SYSTEM_ADMIN — UC-10 chốt Actor duy nhất là System Admin.
