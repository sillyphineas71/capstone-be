...
/**
 * Seed permission notification.reminder.send (Cho phep gui nhac lich hop (UC-144)).
 * Roles: EMPLOYEE, MANAGER, BUSINESS_ADMIN, SYSTEM_ADMIN.
 * KHONG dung role INTERNAL_USER (role nay khong ton tai trong DB).
 */
export class SeedNotificationReminderSendPermission20260718000003 implements MigrationInterface {
  name = 'SeedNotificationReminderSendPermission20260718000003';

  private readonly permission = {
    code: 'notification.reminder.send',
    name: 'Gui nhac lich hop',
    action: 'reminder.send',
    description: 'Cho phep gui nhac lich hop (UC-144)',
  };
