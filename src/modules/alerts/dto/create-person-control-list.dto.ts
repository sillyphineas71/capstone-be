...
export class CreatePersonControlListDto {
  @ApiPropertyOptional({ description: 'ID user hệ thống ứng với người này (nếu đã có tài khoản)' })
  @Expose({ name: 'user_id' })
  @IsOptional()
  @IsUUID()
  userId?: string;

  @ApiPropertyOptional({ description: 'ID face profile dùng để so khớp trực tiếp trên khuôn mặt (độc lập với userId)' })
  @Expose({ name: 'face_profile_id' })
  @IsOptional()
  @IsUUID()
  faceProfileId?: string;

  @ApiProperty({ description: 'Tên hiển thị của người trong danh sách kiểm soát', maxLength: 255 })
  ...
  @ApiPropertyOptional({ description: 'ID ảnh chân dung đính kèm (media_files)' })
  ...
  @ApiPropertyOptional({ description: 'Mức độ ưu tiên (dùng thẳng làm severity khi tạo cảnh báo)', enum: PERSON_CONTROL_PRIORITIES })
  @IsOptional()
  @IsIn(PERSON_CONTROL_PRIORITIES)
  priority?: PersonControlPriority;

  @ApiPropertyOptional({ description: 'Trạng thái kích hoạt theo dõi ngay (mặc định bật)' })
  @IsOptional()
  @IsBoolean()
  active?: boolean;
}
