...
export class AlertRuleResponseDto {
  ...
  @ApiPropertyOptional({ nullable: true, description: 'Khu vực áp dụng quy tắc; null = toàn hệ thống' })
  zone_id: string | null;

  @ApiPropertyOptional({ nullable: true, description: 'Ngưỡng kích hoạt (nếu loại sự kiện cần ngưỡng)' })
  threshold: number | null;

  @ApiProperty({ description: 'Danh sách kênh gửi thông báo (in_app, email, ...)' })
  channels: string[];

  @ApiProperty({ description: 'Trạng thái bật/tắt quy tắc' })
  enabled: boolean;

  @ApiPropertyOptional({ nullable: true, description: 'Khung giờ hạn chế áp dụng quy tắc; null = 24/7' })
  restricted_hours_json: Record<string, unknown> | null;

  @ApiPropertyOptional({ nullable: true, description: 'Danh sách person id được phép bỏ qua cảnh báo' })
  allowed_person_ids_json: string[] | null;

  @ApiPropertyOptional({ nullable: true, description: 'ID user tạo quy tắc' })
  created_by: string | null;

  @ApiPropertyOptional({ nullable: true, description: 'ID user cập nhật gần nhất' })
