 ...
 */
describe('IvssHealthController — RBAC metadata (PROMPT 1)', () => {
  const reflector = new Reflector();

  const cases: Array<[string, string]> = [['health', 'ivss.health.read']];

  it.each(cases)('%s: @RequirePermissions = %s', (method, code) => {
    const handler = (IvssHealthController.prototype as Record<string, unknown>)[
      method
  ...
  it.each(cases)(
    ...
    (method) => {
      const handler = (
        IvssHealthController.prototype as Record<string, unknown>
