 ...
 */
describe('IvssPresenceController — RBAC metadata (PROMPT 1)', () => {
  const reflector = new Reflector();

  const cases: Array<[string, string]> = [
    ['userPresence', 'ivss.presence.read'],
    ['meetingPresence', 'ivss.presence.read'],
    ['report', 'ivss.presence.read'],
  ];

  it.each(cases)('%s: @RequirePermissions = %s', (method, code) => {
    const handler = (
      IvssPresenceController.prototype as Record<string, unknown>
