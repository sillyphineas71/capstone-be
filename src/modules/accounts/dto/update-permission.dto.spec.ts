...
describe('UpdatePermissionDto Validation', () => {
  ...
  it('should accept valid permissionName update', async () => {
    const dto = plainToInstance(UpdatePermissionDto, { permissionName: 'Updated Name' });
    const errors = await validate(dto);
    expect(errors.length).toBe(0);
  });

  it('should accept valid description update', async () => {
    const dto = plainToInstance(UpdatePermissionDto, { description: 'Updated description' });
    const errors = await validate(dto);
    expect(errors.length).toBe(0);
  });
  ...
  it('should reject too long permissionName', async () => {
    const dto = plainToInstance(UpdatePermissionDto, { permissionName: 'A'.repeat(151) });
    const errors = await validate(dto);
