import { Injectable, Logger, NotFoundException, ConflictException, InternalServerErrorException, UnprocessableEntityException } from '@nestjs/common';
...
import { NotificationEntity, NotificationType, NotificationChannel, NotificationDeliveryStatus, NotificationPriority } from '../../notifications/entities/notification.entity.js';
...
export class AdminAvatarReviewService {
  ...
  async listAvatarSubmissions(query: ListAvatarSubmissionsQueryDto) {
    ...
    if (query.departmentId) {
      qb.andWhere('u.departmentId = :departmentId', { departmentId: query.departmentId });
    }

    const sortColumn = SORT_FIELD_MAP[sortBy] ?? 'fp.enrolledAt';
    const order = sortOrder.toUpperCase() === 'ASC' ? 'ASC' : 'DESC';
    qb.orderBy(sortColumn, order as 'ASC' | 'DESC');

    if (sortBy === 'qualityScore') {
      qb.addOrderBy('fp.qualityScore', order as 'ASC' | 'DESC', 'NULLS LAST');
    }
  ...
  async getAvatarSubmissionDetail(faceProfileId: string) {
    ...
    if (!fp) {
      throw new NotFoundException({ code: 'AVATAR_SUBMISSION_NOT_FOUND', message: 'Avatar submission not found.' });
    }

    const user = fp.user as UserEntity | undefined;
    if (!user) {
      throw new NotFoundException({ code: 'USER_NOT_FOUND', message: 'Owning user not found.' });
    }
    ...
    if (fp.primaryImageFileId) {
      const rows: Array<{ file_name: string; mime_type: string; file_size_bytes: string | null; storage_provider: string }> =
        await this.dataSource.manager.query(
          `SELECT file_name, mime_type, file_size_bytes, storage_provider FROM media_files WHERE id = $1 AND deleted_at IS NULL`,
          [fp.primaryImageFileId],
        );
      if (rows.length > 0) {
        imageFile = { fileName: rows[0].file_name, mimeType: rows[0].mime_type, fileSizeBytes: rows[0].file_size_bytes, storageProvider: rows[0].storage_provider };
        hasPreview = true;
      }
    }
  ...
  async getAvatarDownloadUrl(faceProfileId: string, adminUserId: string) {
    ...
    if (!fp) throw new NotFoundException({ code: 'AVATAR_SUBMISSION_NOT_FOUND', message: 'Avatar submission not found.' });
    if (!fp.primaryImageFileId) throw new NotFoundException({ code: 'AVATAR_MEDIA_NOT_FOUND', message: 'Avatar media file not found.' });
    ...
    if (rows.length === 0) throw new NotFoundException({ code: 'AVATAR_MEDIA_NOT_FOUND', message: 'Avatar media file not found.' });

    const baseUrl = this.configService.get<string>('API_PUBLIC_BASE_URL', 'http://localhost:3000');
    const ttl = this.configService.get<number>('MEDIA_DOWNLOAD_TOKEN_TTL_SECONDS', 600);

    let signed: { token: string; expiresAt: string };
    try {
      signed = this.storageService.generateSignedDownloadToken(rows[0].id, ttl);
    } catch (err) {
      this.logger.error(`Failed to generate signed download token: ${err instanceof Error ? err.message : err}`);
      throw new InternalServerErrorException({ code: 'AVATAR_DOWNLOAD_URL_FAILED', message: 'Failed to generate download URL.' });
    }
    ...
    try {
      await this.dataSource.manager.insert(AuditLogEntity, {
        ...
        metadataJson: { targetUserId: fp.userId, mediaFileId: rows[0].id, expiresAt: signed.expiresAt },
      });
    } catch (err) {
      this.logger.warn(`Failed to write audit log for download: ${err instanceof Error ? err.message : err}`);
    }
    ...
    try {
      return await this.dataSource.transaction(async (manager) => {
        ...
        if (fp.status !== FaceProfileStatus.PENDING_REVIEW) {
          throw new ConflictException({ code: 'AVATAR_SUBMISSION_NOT_PENDING', message: 'Avatar submission is not in pending_review status.' });
        }
        ...
        if (!user || user.accountStatus !== 'active' || user.deletedAt) {
          throw new NotFoundException({ code: 'USER_NOT_FOUND', message: 'User not found, inactive, or deleted.' });
        }

        if (!fp.primaryImageFileId) {
          throw new InternalServerErrorException({ code: 'AVATAR_APPROVE_FAILED', message: 'Primary image file ID is missing.' });
        }

        const mediaRows: Array<{ file_url: string | null }> = await manager.query(
          `SELECT file_url FROM media_files WHERE id = $1 AND deleted_at IS NULL`,
          [fp.primaryImageFileId],
        );
        if (mediaRows.length === 0 || !mediaRows[0].file_url) {
          throw new InternalServerErrorException({ code: 'AVATAR_APPROVE_FAILED', message: 'Media file record not found or has no file_url.' });
        }
        ...
        if (oldActive.length > 0) {
          await manager.update(FaceProfileEntity, oldActive[0].id, { status: FaceProfileStatus.REVOKED, lastUpdatedAt: now });
        }

        await manager.update(FaceProfileEntity, faceProfileId, { status: FaceProfileStatus.ACTIVE, lastUpdatedAt: now });
        await manager.update(UserEntity, fp.userId, { avatarUrl: mediaRows[0].file_url });
        ...
        await manager.insert(AuditLogEntity, {
          ...
          newValueJson: { status: FaceProfileStatus.ACTIVE, avatarUrlUpdated: true, oldActiveFaceProfileId: oldActive.length > 0 ? oldActive[0].id : undefined },
          severity: AuditLogSeverity.INFO,
        });

        return { faceProfileId, userId: fp.userId, status: FaceProfileStatus.ACTIVE, approvedAt: now.toISOString() };
      });
    } catch (err) {
      if (err instanceof NotFoundException || err instanceof ConflictException) throw err;
      this.logger.error(`Approve avatar failed: ${err instanceof Error ? err.message : err}`);
      throw new InternalServerErrorException({ code: 'AVATAR_APPROVE_FAILED', message: 'Failed to approve avatar submission.' });
    }
  }

  // ── US4: Reject (transaction + notification) ──────────────────────────

  async rejectAvatarSubmission(faceProfileId: string, reason: string, adminUserId: string) {
    const normalizedReason = reason.trim().normalize('NFC');
    if (normalizedReason.length === 0) {
      throw new UnprocessableEntityException({ code: 'AVATAR_REJECTION_REASON_REQUIRED', message: 'Rejection reason is required.' });
    }
    if (Array.from(normalizedReason).length > 500) {
      throw new UnprocessableEntityException({ code: 'AVATAR_REJECTION_REASON_TOO_LONG', message: 'Rejection reason must not exceed 500 characters.' });
    }
    ...
    try {
      return await this.dataSource.transaction(async (manager) => {
        ...
        await manager.insert(NotificationEntity, {
          ...
          payloadJson: { reason: normalizedReason, reviewedBy: adminUserId, reviewedAt: nowIso },
        });

        return { faceProfileId, userId: fp.userId, status: FaceProfileStatus.REJECTED, rejectedAt: nowIso };
      });
    } catch (err) {
      if (err instanceof NotFoundException || err instanceof ConflictException || err instanceof UnprocessableEntityException) throw err;
      this.logger.error(`Reject avatar failed: ${err instanceof Error ? err.message : err}`);
      throw new InternalServerErrorException({ code: 'AVATAR_REJECT_FAILED', message: 'Failed to reject avatar submission.' });
    }
  }
}