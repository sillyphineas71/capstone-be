...
describe('RolePermissionsService', () => {
  ...
  const mockRole = { id: 'role-uuid', isSystemRole: false, roleCode: 'admin', roleName: 'Admin', isActive: true };
  const mockSystemRole = { id: 'sysrole-uuid', isSystemRole: true, roleCode: 'sysadmin', roleName: 'System Admin', isActive: true };
  const mockPermission = { id: 'perm-uuid', permissionCode: 'meetings.create', isActive: true, moduleCode: 'meetings' };
  const mockAdminPermission = { id: 'admin-perm-uuid', permissionCode: 'admin.manage_permissions', isActive: true, moduleCode: 'admin' };
  ...
  beforeEach(async () => {
    ...
    rpRepo = { find: jest.fn(), findOne: jest.fn(), create: jest.fn(), remove: jest.fn() } as any;
  ...
  describe('findByRole', () => {
    it('[AC-008] should return permissions for a role', async () => {
      ...
      rpRepo.find.mockResolvedValue([{ id: 'rp-uuid', roleId: 'role-uuid', permissionId: 'perm-uuid', grantedBy: 'user-uuid', grantedAt: new Date(), permission: mockPermission } as any]);
    ...
    it('should throw ROLE_NOT_FOUND if role not exist', async () => {
      ...
      await expect(service.findByRole('nonexistent')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('assign', () => {
    it('[AC-006] should assign permissions successfully with transaction', async () => {
      ...
      const result = await service.assign('role-uuid', { permissionIds: ['perm-uuid'] }, 'user-uuid');

      expect(result.assigned).toEqual(['perm-uuid']);
      expect(mockQueryRunner.commitTransaction).toHaveBeenCalled();
      expect(auditLogs.logAction).toHaveBeenCalledWith(expect.objectContaining({ actionType: 'ASSIGN_PERMISSION' }));
    });

    it('[AC-017] should reject inactive permission — fatal rollback', async () => {
      roleRepo.findOne.mockResolvedValue(mockRole as any);
      permRepo.find.mockResolvedValue([{ ...mockPermission, isActive: false } as any]);

      await expect(
        service.assign('role-uuid', { permissionIds: ['perm-uuid'] }, 'user-uuid'),
    ...
    it('should reject non-existent permissionId — fatal rollback', async () => {
      ...
      await expect(
        service.assign('role-uuid', { permissionIds: ['nonexistent-uuid'] }, 'user-uuid'),
    ...
    it('should reject non-existent roleId — fatal', async () => {
      ...
      await expect(
        service.assign('nonexistent', { permissionIds: ['perm-uuid'] }, 'user-uuid'),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe('revoke', () => {
    it('[AC-007] should revoke permission from role', async () => {
      roleRepo.findOne.mockResolvedValue(mockRole as any);
      rpRepo.findOne.mockResolvedValue({ id: 'rp-uuid', roleId: 'role-uuid', permissionId: 'perm-uuid', permission: mockPermission } as any);
      ...
      expect(auditLogs.logAction).toHaveBeenCalledWith(expect.objectContaining({ actionType: 'REVOKE_PERMISSION' }));
    });
    ...
    it('[AC-018] should reject revoking admin permission from system role', async () => {
      ...
      rpRepo.findOne.mockResolvedValue({ id: 'rp-uuid', permission: mockAdminPermission } as any);
    ...
    it('[AC-019] should allow revoking non-admin permission from system role', async () => {
      roleRepo.findOne.mockResolvedValue(mockSystemRole as any);
      rpRepo.findOne.mockResolvedValue({ id: 'rp-uuid', permission: mockPermission } as any);
      rpRepo.remove.mockResolvedValue({} as any);
