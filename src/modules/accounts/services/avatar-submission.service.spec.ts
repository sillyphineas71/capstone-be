...
import {
  ...
  UploadedAvatarFile,
} from './avatar-submission.service.js';

/**
 * ACCT-AVATAR-SUBMIT-001 — Unit test cho AvatarSubmissionService (US2).
...
const makeFile = (
  over: Partial<UploadedAvatarFile> = {},
): UploadedAvatarFile => ({
  buffer: jpegBuffer(),
  originalname: 'me.jpg',
  mimetype: 'image/jpeg',
...
describe('AvatarSubmissionService', () => {
  let service: AvatarSubmissionService;
  ...
  beforeEach(() => {
    ...
    service = new AvatarSubmissionService(
      userRepo as never,
  ...
  it('happy lần đầu (chưa có row) → pending_review + audit avatar.upload (AC-002)', async () => {
    ...
    expect(audit?.actionType).toBe('avatar.upload');
  });

  it('reupload sau rejected → pending_review + audit avatar.reupload (AC-005)', async () => {
    faceProfileRepo.find.mockResolvedValue([{ id: 'old', status: 'rejected' }]);
    const result = await service.submit('u1', makeFile(), true);

    expect(result.avatarReviewStatus).toBe('pending_review');
    const audit = insertCalls.find((c) => 'actionType' in c);
    expect(audit?.actionType).toBe('avatar.reupload');
  });

  it('replace khi approved (row active) → tạo row mới pending, audit avatar.reupload (AC-006b)', async () => {
  ...
  it('thiếu file → AVATAR_FILE_REQUIRED (AC-007)', async () => {
    await expect(service.submit('u1', undefined, true)).rejects.toMatchObject({
      response: { code: 'AVATAR_FILE_REQUIRED' },
    });
    expect(cloudinary.uploadImage).not.toHaveBeenCalled();
  });

  it('magic bytes sai (PDF) → AVATAR_FILE_TYPE_INVALID (AC-008)', async () => {
    const pdf = makeFile({ buffer: Buffer.from('%PDF-1.7', 'ascii') });
    await expect(service.submit('u1', pdf, true)).rejects.toMatchObject({
      response: { code: 'AVATAR_FILE_TYPE_INVALID' },
    });
  });

  it('file quá lớn → AVATAR_FILE_TOO_LARGE (AC-009)', async () => {
    ...
    await expect(service.submit('u1', big, true)).rejects.toMatchObject({
      response: { code: 'AVATAR_FILE_TOO_LARGE' },
    });
  });

  it('consent false → AVATAR_CONSENT_REQUIRED, không upload (AC-010)', async () => {
    await expect(service.submit('u1', makeFile(), false)).rejects.toMatchObject(
      { response: { code: 'AVATAR_CONSENT_REQUIRED' } },
    );
    expect(cloudinary.uploadImage).not.toHaveBeenCalled();
  });
  ...
  it('Cloudinary upload lỗi → AVATAR_STORAGE_FAILED, không mở transaction (ERR-009)', async () => {
  ...
  it('transaction lỗi thường → cleanup Cloudinary + AVATAR_UPLOAD_FAILED (EC-004)', async () => {
    dataSource.transaction.mockRejectedValue(new Error('db write failed'));
    await expect(service.submit('u1', makeFile(), true)).rejects.toBeInstanceOf(
      InternalServerErrorException,
    );
    expect(cloudinary.deleteImage).toHaveBeenCalledWith('avatars/x');
  });

  it('transaction unique violation → AVATAR_ALREADY_PENDING_REVIEW, KHÔNG cleanup (EC-003)', async () => {
    ...
    await expect(service.submit('u1', makeFile(), true)).rejects.toMatchObject({
      response: { code: 'AVATAR_ALREADY_PENDING_REVIEW' },
    });
