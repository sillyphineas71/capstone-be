...
import { RoomUsageDashboardResponseDto, RoomDetailResponseDto } from '../dto/room-usage-response.dto';
...
export class RoomUsageDashboardController {
  ...
  @ApiOperation({
    ...
    description: 'Tra ve danh sach cac phong kem theo thong so dat phong, su dung thuc te va bieu do trend.',
  })
  @ApiResponse({
    status: 200,
  ...
  async getComparison(
    ...
    try {
      const { data, message } = await this.service.getComparisonDashboard(currentUser, query);
      return {
        success: true,
        message,
        data,
  ...
  @ApiOperation({
    ...
    description: 'Tra ve thong ke chi tiet, heatmap 24 gio va danh sach cac cuoc hop cua phong.',
  })
  @ApiResponse({
    status: 200,
  ...
  async getDetail(
    ...
    try {
      const { data, message } = await this.service.getRoomDetail(currentUser, roomId, query);
      return {
        success: true,
        message,
        data,
