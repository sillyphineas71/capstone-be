...
describe('PTA decorated endpoints', () => {
  ...
  it('marks meeting read endpoints that filter by meeting participant', () => {
    expect(
      Reflect.getMetadata(
        ALLOW_PARTNER_ACCOUNT_KEY,
        MeetingsController.prototype.getMeetingById,
      ),
    ).toBe(true);
    expect(
      Reflect.getMetadata(
        ALLOW_PARTNER_ACCOUNT_KEY,
        MeetingsController.prototype.getAgendas,
      ),
    ).toBe(true);
  });
  ...
  it('marks live meeting read endpoints that filter by meeting participant', () => {
    expect(
      Reflect.getMetadata(
        ALLOW_PARTNER_ACCOUNT_KEY,
        LiveMeetingController.prototype.listNotes,
      ),
    ).toBe(true);
    expect(
      Reflect.getMetadata(
        ALLOW_PARTNER_ACCOUNT_KEY,
        LiveMeetingController.prototype.getMeetingTimeline,
      ),
    ).toBe(true);
  });
});
