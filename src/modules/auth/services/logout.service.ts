  ...
  async logout(jti: string, exp: number): Promise<void> {
    ...
    if (ttlMillis > 0) {
      try {
        ...
        throw new InternalServerErrorException('Cannot process logout at the moment');
      }
    }
  }

  async logLogoutAudit(userId: string, jti: string, req: Request): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to insert audit log for logout: ${error.message}`, error.stack);
