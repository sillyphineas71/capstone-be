...
import { RecordingConfigController } from './recording-config.controller.js';
 ...
 */
describe('RecordingConfigController — RBAC metadata (PROMPT 1)', () => {
  const reflector = new Reflector();

  const cases: Array<[string, string]> = [
    ['create', 'recording.config.create'],
    ['findOne', 'recording.config.read'],
    ['update', 'recording.config.update'],
  ];

  it.each(cases)('%s: @RequirePermissions = %s', (method, code) => {
    const handler = (
      RecordingConfigController.prototype as Record<string, unknown>
