...
describe('LiveMeetingController - endMeeting', () => {
  ...
  describe('getPresentAttendees', () => {
    ...
    it('should return present attendees list with correct shape', async () => {
      mockLiveMeetingService.getPresentAttendees.mockResolvedValue(mockResponse);
    ...
    it('should return 200 with limited fields for participant', async () => {
      const participantResponse = {
        data: {
          ...
          occupancyCount: 2,
          presentUsers: [
            { userId: 'u1', fullName: 'A', presenceStatus: 'present', presenceSource: null },
          ],
          updatedAt: new Date().toISOString(),
        },
        meta: { page: 1, limit: 20, total: 2, totalPages: 1 },
      };
      mockLiveMeetingService.getPresentAttendees.mockResolvedValue(participantResponse);
