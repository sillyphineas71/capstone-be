...
describe('LiveMeetingService - scheduleWarningJob', () => {
  ...
  beforeEach(async () => {
    ...
    mockBgJobRepo = { findOne: jest.fn(), create: jest.fn(), save: jest.fn().mockResolvedValue({}), update: jest.fn().mockResolvedValue({ affected: 1 }) };
    mockEventRepo = { create: jest.fn(), save: jest.fn().mockResolvedValue({}) };
    mockQueue = { add: jest.fn().mockResolvedValue({ id: 'mock-job-id' }), getJob: jest.fn() };
    ...
    const dataSourceMock = {
      getRepository: jest.fn().mockImplementation((entity: any) => {
        ...
        if (entity === SystemConfigEntity) return { findOne: jest.fn().mockResolvedValue(null) };
  ...
  it('T-W02 [Scenario 3] AF2 should adjust warning time when remainingMinutes < config', async () => {
    ...
    expect(result.warningScheduledAt!.getTime()).toBeGreaterThanOrEqual(expectedMin);
    expect(result.warningScheduledAt!.getTime()).toBeLessThanOrEqual(expectedMax);
  });
  ...
  it('T-W06 [ERR-04] should skip when meeting not in_progress', async () => {
    mockMeetingRepo.findOne.mockResolvedValue({ ...inProgressMeeting, status: 'completed' });
  ...
  it('T-W07 [ERR-05] should skip when end_time is null', async () => {
    mockMeetingRepo.findOne.mockResolvedValue({ ...inProgressMeeting, endTime: null });
...
describe('LiveMeetingService - cancelWarningJob', () => {
  ...
  beforeEach(async () => {
    mockBgJobRepo = { findOne: jest.fn(), update: jest.fn().mockResolvedValue({ affected: 1 }) };
  ...
  it('T-W15 [Scenario 4] should cancel warning and update background_jobs to CANCELLED', async () => {
    ...
    mockBgJobRepo.findOne.mockResolvedValue({ id: 'bg-1', status: 'scheduled' });

    await service.cancelWarningJob('m-warn-001');

    expect(mockQueue.getJob).toHaveBeenCalledWith('meeting-time-warning:m-warn-001');
...
} from '../services/meeting-warning.service.js';
import {
  MeetingEntity, MeetingStatus,
} from '../../meetings/entities/meeting.entity.js';
import {
  MeetingEventEntity, MeetingEventType, MeetingEventSourceType,
} from '../../meetings/entities/meeting-event.entity.js';
import {
  MeetingParticipantEntity,
} from '../../meetings/entities/meeting-participant.entity.js';
import {
  RoomBookingEntity, RoomBookingStatus,
} from '../../rooms/entities/room-booking.entity.js';
import {
  NotificationEntity, NotificationType, NotificationPriority, NotificationDeliveryStatus,
} from '../../notifications/entities/notification.entity.js';
import {
  BackgroundJobEntity, BackgroundJobStatus,
...
describe("MeetingWarningService - processWarningJob", () => {
  ...
  beforeEach(async () => {
    mockMeetingRepo = { findOne: jest.fn() };
    mockEventRepo = { findOne: jest.fn(), create: jest.fn(), save: jest.fn().mockResolvedValue({ id: "evt-001" }) };
    mockNotificationRepo = { findOne: jest.fn(), save: jest.fn().mockResolvedValue({ id: "notif-001" }) };
    mockBgJobRepo = { findOne: jest.fn(), update: jest.fn().mockResolvedValue({ affected: 1 }) };
  ...
  describe("Suite A - Guard and Idempotency", () => {
    ...
    it("T-P02 [FR-018, ERR-002, AC-003] should skip when meeting completed", async () => {
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, status: MeetingStatus.COMPLETED });
    ...
    it("T-P03 [FR-018, ERR-002] should skip when meeting cancelled", async () => {
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, status: MeetingStatus.CANCELLED });
      const r = await service.processWarningJob({ meetingId: "mm-warn-001" });
      expect(r.skipped).toBe(true);
      expect(r.reason).toBe("meeting_not_in_progress");
    });
    ...
    it("T-P04 [FR-033, NFR-007, AC-008] should skip when warning_sent exists", async () => {
      ...
      mockEventRepo.findOne.mockResolvedValue({ id: "evt-exists", meetingId: "mm-warn-001", eventType: MeetingEventType.WARNING_SENT });
    ...
    it("T-P04b [FR-033, NFR-007] dual-source guard via notification", async () => {
      ...
      mockNotificationRepo.findOne.mockResolvedValue({ id: "notif-exists", relatedEntityId: "mm-warn-001" });
      const r = await service.processWarningJob({ meetingId: "mm-warn-001" });
      expect(r.skipped).toBe(true);
      expect(r.reason).toBe("already_sent");
    });
  ...
  describe("Suite B - remainingMinutes and Late Job", () => {
    it("T-P06 [FR-002, BR-11] remainingMinutes = 10", async () => {
      ...
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      mockNotificationRepo.save.mockResolvedValue({ id: "notif-006" });
      mockBookingRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder(null));
      const r = await service.processWarningJob({ meetingId: "mm-warn-001" });
    ...
    it("T-P07 [FR-031, BR-13, AC-009] late job clamp 0", async () => {
      ...
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: pe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
    ...
    it("T-P08 [FR-031] boundary remainingMinutes = 0", async () => {
      ...
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: ee, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
  ...
  describe("Suite C - Conflict Detection", () => {
    ...
    it("T-P10 [FR-011, AC-002] should return Branch B when next booking at exact endTime", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const nb = { id: "bk-001", meetingId: "other", roomId: "room-001", reservedStartTime: fe, status: RoomBookingStatus.APPROVED };
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      ...
      const sp: any = mockNotificationRepo.save.mock.calls[0][0];
      expect(sp.notificationType).toBe(NotificationType.MEETING_TIME_CONFLICT_WARNING);
    });

    it("T-P11 [FR-003, AC-011] should detect conflict within buffer window", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const b3 = new Date(fe.getTime() + 3 * 60 * 1000);
      const nb = { id: "bk-002", meetingId: "other", roomId: "room-001", reservedStartTime: b3, status: RoomBookingStatus.PENDING };
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
    ...
    it("T-P12 [FR-003, AC-011] should NOT detect conflict outside buffer window", async () => {
      ...
      const nb = { id: "bk-003", meetingId: "other", roomId: "room-001", reservedStartTime: b6, status: RoomBookingStatus.APPROVED };
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "5" });
    ...
    it("T-P13 [FR-015, AC-004] should skip conflict query when roomId is null", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001", roomId: null });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
  ...
  describe("Suite D+E - warningLevel and Notification Payload", () => {
    ...
    it("T-P17 [FR-032, BR-14, AC-009] warningLevel=overdue for Branch A + 0", async () => {
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: new Date(Date.now() - 1000), hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      mockBookingRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder(null));
    ...
    it("T-P18 [FR-011, AC-002] warningLevel=strict for Branch B", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const nb = { id: "bk-wl", meetingId: "other", roomId: "room-001", reservedStartTime: fe, status: RoomBookingStatus.APPROVED };
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      mockBookingRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder(nb));
    ...
    it("T-P19 [FR-032, BR-15, AC-009] warningLevel=urgent for Branch B + 0", async () => {
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: new Date(Date.now() - 1000), hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      const nb = { id: "bk-urg", meetingId: "other", roomId: "room-001", reservedStartTime: new Date(Date.now() - 1000), status: RoomBookingStatus.APPROVED };
      mockBookingRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder(nb));
    ...
    it("T-P21 [FR-011, AC-002] Branch B payload fields", async () => {
      ...
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      const nb = { id: "bk-p21", meetingId: "other", roomId: "room-001", reservedStartTime: fe, status: RoomBookingStatus.APPROVED };
      mockBookingRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder(nb));
    ...
    it("T-P22 [FR-011] Branch B priority=high, Branch A normal", async () => {
      ...
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      const nb2 = { id: "bk-p22", meetingId: "other", roomId: "room-001", reservedStartTime: fe, status: RoomBookingStatus.APPROVED };
  ...
  describe("Suite F+G - WebSocket + Post-notification Writes", () => {
    it("T-P24 [FR-012, BR-17, AC-010] Host receives emitToUser with fields", async () => {
      ...
      await service.processWarningJob({ meetingId: "mm-warn-001" });
      expect(mockWebsocketService.emitToUser).toHaveBeenCalledWith("host-001", "meeting.time.warning", expect.objectContaining({
    ...
    it("T-P25 [FR-012, BR-17, AC-010] Participants get safe payload", async () => {
      ...
      expect(mockWebsocketService.emitToRoom).toHaveBeenCalledWith("meeting:mm-warn-001", "meeting.time.warning", expect.any(Object));
    ...
    it("T-P26 [FR-021, ERR-007, AC-007] WS failure is non-critical", async () => {
      mockWebsocketService.emitToUser.mockImplementation(() => { throw new Error("WS err"); });
      mockWebsocketService.emitToRoom.mockImplementation(() => { throw new Error("WS err"); });
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      mockMeetingRepo.findOne.mockResolvedValue({ ...baseMeeting, endTime: fe, hostId: "host-001" });
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      mockBookingRepo.createQueryBuilder.mockReturnValue(mockQueryBuilder(null));
      mockNotificationRepo.save.mockResolvedValue({ id: "notif-026" });
      mockBgJobRepo.findOne.mockResolvedValue({ id: "bg-026", status: BackgroundJobStatus.SCHEDULED });
      const r = await service.processWarningJob({ meetingId: "mm-warn-001" });
      expect(r.skipped).toBe(false);
    });
    ...
    it("T-P27 [FR-007, FR-009] bg jobs updated to completed", async () => {
      ...
      mockBgJobRepo.findOne.mockResolvedValue({ id: "bg-027", status: BackgroundJobStatus.SCHEDULED });
      await service.processWarningJob({ meetingId: "mm-warn-001" });
      expect(mockBgJobRepo.update).toHaveBeenCalled();
    ...
    it("T-P30 [ERR-005, AC-006] Notification fail throws + bg=failed", async () => {
      ...
      await expect(service.processWarningJob({ meetingId: "mm-warn-001" })).rejects.toThrow();
      expect(mockBgJobRepo.update).toHaveBeenCalled();
...
describe("MeetingWarningService - resolveHost and readConflictBufferConfig", () => {
  ...
  describe("Suite H - resolveHost", () => {
    it("T-P33 [FR-034] should return hostId directly when set", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const m = { id: "mm-h-001", status: MeetingStatus.IN_PROGRESS, endTime: fe, roomId: null, hostId: "direct-host", organizerId: "org", deletedAt: null };
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      const mRepo = { findOne: jest.fn().mockResolvedValue(m) };
      const nRepo = { findOne: jest.fn().mockResolvedValue(null), save: jest.fn().mockResolvedValue({ id: "not-h33" }) };
      const eRepo = { findOne: jest.fn().mockResolvedValue(null), create: jest.fn(), save: jest.fn().mockResolvedValue({}) };
      const bRepo = { findOne: jest.fn().mockResolvedValue({ id: "bg-h33" }), update: jest.fn() };
      const ds = {
        getRepository: jest.fn().mockImplementation((entity: any) => {
      ...
      const mod: TestingModule = await Test.createTestingModule({
        providers: [MeetingWarningService, { provide: DataSource, useValue: ds }, { provide: WebsocketService, useValue: mockWebsocketService }, { provide: ConfigService, useValue: mockConfigService }],
      }).compile();
    ...
    it("T-P34 [FR-034] should resolve host from participants when hostId null", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      const m = { id: "mm-h-002", status: MeetingStatus.IN_PROGRESS, endTime: fe, roomId: null, hostId: null, organizerId: "org", deletedAt: null };
      mockParticipantRepo.findOne.mockResolvedValue({ userId: "host-from-participant" });
      const mRepo = { findOne: jest.fn().mockResolvedValue(m) };
      const nRepo = { findOne: jest.fn().mockResolvedValue(null), save: jest.fn().mockResolvedValue({ id: "not-h34" }) };
      const eRepo = { findOne: jest.fn().mockResolvedValue(null), create: jest.fn(), save: jest.fn().mockResolvedValue({}) };
      const bRepo = { findOne: jest.fn().mockResolvedValue({ id: "bg-h34" }), update: jest.fn() };
      const ds = {
        getRepository: jest.fn().mockImplementation((entity: any) => {
          if (entity === MeetingEntity) return mRepo;
          if (entity === MeetingEventEntity) return eRepo;
    ...
    it("T-P35 [FR-034] should skip when no host found", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "0" });
      const m = { id: "mm-h-003", status: MeetingStatus.IN_PROGRESS, endTime: fe, roomId: null, hostId: null, organizerId: "org", deletedAt: null };
  ...
  describe("Suite I - readConflictBufferConfig", () => {
    it("T-P36 [FR-035] should return 5 when config is 5", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const m = { id: "mm-c-001", status: MeetingStatus.IN_PROGRESS, endTime: fe, roomId: null, hostId: "host-001", organizerId: "org", deletedAt: null };
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "5" });
      const mRepo = { findOne: jest.fn().mockResolvedValue(m) };
      const nRepo = { findOne: jest.fn().mockResolvedValue(null), save: jest.fn().mockResolvedValue({ id: "not-c36" }) };
      const eRepo = { findOne: jest.fn().mockResolvedValue(null), create: jest.fn(), save: jest.fn().mockResolvedValue({}) };
      const bRepo = { findOne: jest.fn().mockResolvedValue({ id: "bg-c36" }), update: jest.fn() };
      const ds = {
        getRepository: jest.fn().mockImplementation((entity: any) => {
          if (entity === MeetingEntity) return mRepo;
          if (entity === MeetingEventEntity) return eRepo;
    ...
    it("T-P37 [FR-035] should return 0 when config not found", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const m = { id: "mm-c-002", status: MeetingStatus.IN_PROGRESS, endTime: fe, roomId: null, hostId: "host-001", organizerId: "org", deletedAt: null };
      mockConfigRepo.findOne.mockResolvedValue(null);
      const mRepo = { findOne: jest.fn().mockResolvedValue(m) };
      const nRepo = { findOne: jest.fn().mockResolvedValue(null), save: jest.fn().mockResolvedValue({ id: "not-c37" }) };
      const eRepo = { findOne: jest.fn().mockResolvedValue(null), create: jest.fn(), save: jest.fn().mockResolvedValue({}) };
      const bRepo = { findOne: jest.fn().mockResolvedValue({ id: "bg-c37" }), update: jest.fn() };
      const ds = {
        getRepository: jest.fn().mockImplementation((entity: any) => {
          if (entity === MeetingEntity) return mRepo;
          if (entity === MeetingEventEntity) return eRepo;
    ...
    it("T-P38 [FR-035, ERR-004c] should return 0 when unparseable", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const m = { id: "mm-c-003", status: MeetingStatus.IN_PROGRESS, endTime: fe, roomId: null, hostId: "host-001", organizerId: "org", deletedAt: null };
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "abc" });
      const mRepo = { findOne: jest.fn().mockResolvedValue(m) };
      const nRepo = { findOne: jest.fn().mockResolvedValue(null), save: jest.fn().mockResolvedValue({ id: "not-c38" }) };
      const eRepo = { findOne: jest.fn().mockResolvedValue(null), create: jest.fn(), save: jest.fn().mockResolvedValue({}) };
      const bRepo = { findOne: jest.fn().mockResolvedValue({ id: "bg-c38" }), update: jest.fn() };
      const ds = {
        getRepository: jest.fn().mockImplementation((entity: any) => {
          if (entity === MeetingEntity) return mRepo;
          if (entity === MeetingEventEntity) return eRepo;
    ...
    it("T-P39 [FR-035, ERR-004c] should return 0 when negative", async () => {
      const fe = new Date(Date.now() + 10 * 60 * 1000);
      const m = { id: "mm-c-004", status: MeetingStatus.IN_PROGRESS, endTime: fe, roomId: null, hostId: "host-001", organizerId: "org", deletedAt: null };
      mockConfigRepo.findOne.mockResolvedValue({ configValue: "-3" });
      const mRepo = { findOne: jest.fn().mockResolvedValue(m) };
      const nRepo = { findOne: jest.fn().mockResolvedValue(null), save: jest.fn().mockResolvedValue({ id: "not-c39" }) };
      const eRepo = { findOne: jest.fn().mockResolvedValue(null), create: jest.fn(), save: jest.fn().mockResolvedValue({}) };
      const bRepo = { findOne: jest.fn().mockResolvedValue({ id: "bg-c39" }), update: jest.fn() };
      const ds = {
        getRepository: jest.fn().mockImplementation((entity: any) => {
          if (entity === MeetingEntity) return mRepo;
          if (entity === MeetingEventEntity) return eRepo;
