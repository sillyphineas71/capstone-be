 ...
 */
describe('NoShowConfigController — RBAC metadata (PROMPT 1)', () => {
  const reflector = new Reflector();

  const cases: Array<[string, string]> = [
    ['get', 'room.noshow.configure'],
    ['update', 'room.noshow.configure'],
  ];

  it.each(cases)('%s: @RequirePermissions = %s', (method, code) => {
    const handler = (
      NoShowConfigController.prototype as Record<string, unknown>
