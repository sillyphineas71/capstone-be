 ...
 */
describe('EarlyVacancyConfigController — RBAC metadata (PROMPT 1)', () => {
  const reflector = new Reflector();

  const cases: Array<[string, string]> = [
    ['get', 'room.early_vacancy.configure'],
    ['update', 'room.early_vacancy.configure'],
  ];

  it.each(cases)('%s: @RequirePermissions = %s', (method, code) => {
    const handler = (
      EarlyVacancyConfigController.prototype as Record<string, unknown>
