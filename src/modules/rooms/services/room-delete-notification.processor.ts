 ...
 * gui email cho organizer cua tung meeting bi anh huong.
 *
...
export class RoomDeleteNotificationProcessor {
  ...
  private async processOneMeeting(meetingId: string): Promise<void> {
    ...
    const organizer = await this.dataSource
      ...
      .findOne({ where: { id: meeting.organizerId } });
    if (!organizer?.email) {
      // Khong co email de gui — coi nhu khong the thong bao, khong throw
      // (khong phai loi he thong, chi la thieu du lieu lien lac).
      this.logger.warn(
        `[RoomDeleteNotify] Meeting ${meetingId}: organizer khong co email, bo qua gui thong bao.`,
      );
      return;
    }
