 ...
 */
describe('UnmappedReviewController — RBAC metadata (PROMPT 1)', () => {
  const reflector = new Reflector();

  const cases: Array<[string, string]> = [
    ['list', 'face.unmapped.read'],
    ['map', 'face.unmapped.map'],
  ];

  it.each(cases)('%s: @RequirePermissions = %s', (method, code) => {
    const handler = (
      UnmappedReviewController.prototype as Record<string, unknown>
