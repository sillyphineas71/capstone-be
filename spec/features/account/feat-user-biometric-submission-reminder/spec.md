# Feature Specification: Nhắc nhở và tự nộp ảnh sinh trắc học (User Biometric Submission Reminder)

- **Feature ID**: ACCT-BIOMETRIC-SUBMIT-001
- **Feature Name**: Nhắc nhở và tự nộp ảnh sinh trắc học (phía user) [SỬA 2026-07-29: đổi từ "ảnh đại diện/khuôn mặt" — tránh nhầm lẫn với avatar hiển thị, xem `feat-split-avatar-and-biometric/plan.md`]
...
- **Source Documents**:
  ...
  - Feature song song (do agent khác phụ trách, chỉ tham chiếu phụ thuộc): `feat-admin-biometric-review-workflow` (chưa tồn tại tại thời điểm viết spec này).

> **Gợi ý đặt tên folder feature**: tên `feat-user-biometric-submission-reminder` (đã dùng làm đường dẫn của file này) là phù hợp vì bao trùm đúng 2 hành vi cốt lõi của feature — "reminder" (popup nhắc theo trạng thái) và "submission" (user tự nộp/nộp lại ảnh). Một phương án thay thế cũng hợp lý nếu muốn nhấn mạnh tính "self-service" để phân biệt rõ hơn với UC-17 (admin-driven) là `feat-self-biometric-upload-and-reminder`. Khuyến nghị: giữ nguyên `feat-user-biometric-submission-reminder` như đã tạo, không đổi lại trừ khi team có convention khác.
...
| :--- | :--- | :--- |
| 2026-06-24 | Tạo spec lần đầu cho ACCT-BIOMETRIC-SUBMIT-001 (Nhắc nhở và tự nộp ảnh đại diện/khuôn mặt) | Toàn bộ file |
| 2026-06-24 | Thống nhất model face_profiles theo submission: mỗi lần submit tạo row mới pending_review; old active giữ nguyên; approved user submit thay thế tạo row pending mới thay vì update row active; chuẩn hóa audit action biometric.upload/biometric.reupload; cập nhật BR-008, BR-009, BR-010, BR-012, FR-006, FR-009, FR-025, FR-DATA-002, FR-DATA-003, NFR-008, EC-003, mục 18.5, 18.6, 20.1 | BR-008, BR-009, BR-010, BR-012, FR-006, FR-009, FR-025, FR-DATA-002, FR-DATA-003, NFR-008, EC-003, mục 18.5, 18.6, 20.1 |
| 2026-06-24 | Giải quyết toàn bộ clarify pass (BL-01..03, AR-01..02, VL-01..02, DM-01..02, EH-01..02, SB-01..02): thêm biometric status resolution priority; consent_at chỉ ghi vào row mới; rule sinh profile_code dùng chung; permission seed qua migration idempotent; chỉ SYSTEM_ADMIN approve biometric; consentAccepted transform cho multipart; MIME validate bằng magic bytes; partial unique index ux_face_profiles_user_pending bắt buộc; pre-generate UUID + insert order media_files/face_profiles; best-effort cleanup Cloudinary orphan; validation/error precedence; auth dùng raw SQL không import AccountsService; popup không throttle | BR-004..006, BR-010, BR-011, BR-016, BR-PROFILE-CODE (mới), BR-REMINDER-FREQUENCY (mới), FR-001, FR-004..007, FR-011, FR-015, FR-017, mục 9, 10, 11 (thêm 11.2), 14 (AC-003, AC-003b mới, AC-006b, AC-008, AC-010, AC-010b mới, AC-015, AC-016/017 mới), 15 (EC-003..005, EC-007 mới), 16 (NFR-004, NFR-008, NFR-010), 17, 18 (banner, 18.2, 18.5, 18.6, 18.7 mới), 20 (thêm 20.4) |
| 2026-07-29 | T09 (feat-split-avatar-and-biometric/tasks.md): đổi folder + toàn bộ nội dung từ "avatar" sang "biometric" đúng bản chất (Feature ID ACCT-BIOMETRIC-SUBMIT-001, endpoint `/me/biometric-status`+`/me/biometric-submission`, error code `BIOMETRIC_*`, audit action `biometric.upload/reupload`, field response `biometricReviewStatus`/`biometricRequired`/`shouldShowBiometricPopup`, permission `profile.biometric.read_status`/`profile.biometric.submit`). Sửa BR-002/BR-003/FR-002/FR-009/AC-006/AR-02 — các chỗ trước đây ngầm định `avatarUrl`/`users.avatar_url` sẽ được cập nhật khi biometric approve; nay xác nhận `avatar_url` hoàn toàn độc lập, do feature `feat-update-avatar-photo` quản lý (quyết định D2, `feat-split-avatar-and-biometric/plan.md`). Field `avatarUrl` trong response GET biometric-status/login GIỮ NGUYÊN tên, chỉ đổi ý nghĩa nguồn dữ liệu. | Header, 1, 2, BR-002, BR-003, FR-002, FR-005, FR-009, mục 8 (API Contract), mục 11 (Error Handling), mục 12 (Audit), mục 14 (AC-006, bảng traceability AR-02), toàn bộ tài liệu (rename) |
...
1. **Reminder**: Mỗi lần user đăng nhập, hệ thống tính toán trạng thái biometric/face profile hiện tại của chính user đó và trả về các field derived (`biometricReviewStatus`, `biometricRequired`, `shouldShowBiometricPopup`) để frontend quyết định có hiển thị popup nhắc nhở hay không. Popup chỉ mang tính nhắc nhở, không chặn user sử dụng hệ thống.
2. **Submission**: User tự upload ảnh khuôn mặt (dữ liệu sinh trắc học) của chính mình. Ảnh được lưu trên Cloudinary (MVP), backend tạo metadata trong `media_files` và tạo/cập nhật `face_profiles` với trạng thái `pending_review`.

Feature này **không** xử lý việc admin approve/reject — phần đó thuộc feature song song `feat-admin-biometric-review-workflow`. Feature này cũng **không** triển khai face recognition/embedding/AI pipeline thật, đúng theo giới hạn MVP của `AGENTS.md`.

[SUA 2026-07-29] Feature này **hoàn toàn độc lập** với avatar hiển thị (`users.avatar_url`) — xem `spec/features/account/feat-split-avatar-and-biometric/plan.md` (quyết định D2/D3) và `spec/features/account/feat-update-avatar-photo/spec.md` (feature quản lý avatar, tự do, không cần duyệt). Ảnh sinh trắc học nộp ở feature này KHÔNG BAO GIỜ trở thành avatar hiển thị, kể cả sau khi được admin approve.
...
- API cho user xem trạng thái biometric/face profile của chính mình.
- API cho user tự upload/re-upload biometric.
...
- Admin approve/reject biometric (thuộc `feat-admin-biometric-review-workflow`).
...
- Email notification.
- Chặn user sử dụng hệ thống khi chưa có biometric.
...
|---|---|---|
| Internal Employee (System Role `INTERNAL_USER`) | Self-service: xem trạng thái và tự nộp biometric của chính mình | `profile.biometric.read_status`, `profile.biometric.submit` |
| Manager (System Role `MANAGER`) | Tương tự Internal Employee, áp dụng cho chính tài khoản Manager | `profile.biometric.read_status`, `profile.biometric.submit` |
| Business Admin (System Role `BUSINESS_ADMIN`) | Tự quản lý biometric của chính mình giống mọi user khác (không phải hành vi quản trị) | `profile.biometric.read_status`, `profile.biometric.submit` |
| System Admin (System Role `SYSTEM_ADMIN`) | Tự quản lý biometric của chính mình; là actor duy nhất có quyền approve/reject ở feature khác (không thuộc scope này) | `profile.biometric.read_status`, `profile.biometric.submit` |

### 4.1 Actor Constraints

- Phải đăng nhập thành công (JWT access token hợp lệ).
- Chỉ thao tác trên dữ liệu biometric/face profile của **chính mình** — không có tham số `userId` truyền từ client; user id luôn lấy từ token.
...
BR-001: Chỉ chính user (qua JWT) mới được xem/nộp biometric của bản thân; không hỗ trợ tham số target user khác trong feature này (khác với UC-17 admin-driven).

BR-002: Trạng thái biometric hiển thị cho user (`biometricReviewStatus`) được suy ra từ bản ghi `face_profiles` hiện tại của user, KHÔNG dùng `users.avatar_url` để xác định workflow status.

BR-003 [SỬA 2026-07-29]: `users.avatar_url` là dữ liệu avatar hiển thị, quản lý hoàn toàn độc lập bởi feature `feat-update-avatar-photo` — KHÔNG liên quan đến kết quả approve biometric. Feature này (submit mới) KHÔNG được set/update `users.avatar_url` trong bất kỳ trường hợp nào. (Bản cũ của rule này mô tả `avatar_url` "chỉ chứa biometric đã được admin approve" — không còn đúng sau khi tách avatar/biometric, xem `feat-split-avatar-and-biometric/plan.md` quyết định D2.)

BR-004: **Biometric status resolution priority.** Vì một user có thể có NHIỀU row `face_profiles` theo lịch sử submission (mỗi lần submit tạo row mới — BR-008/BR-009), `biometricReviewStatus` PHẢI được tính theo đúng thứ tự ưu tiên sau, áp dụng trên TOÀN BỘ row `face_profiles` của user (KHÔNG dùng khái niệm "một row hiện hành" đơn lẻ):
  1. Nếu user có ÍT NHẤT một row `face_profiles` với `status = pending_review` và `deleted_at IS NULL` → `biometricReviewStatus = pending_review`.
  2. Ngược lại, nếu user có ÍT NHẤT một row `face_profiles` với `status = active` và `deleted_at IS NULL` → `biometricReviewStatus = approved`.
  3. Ngược lại, nếu row `face_profiles` gần nhất liên quan (xác định theo `last_updated_at`, hoặc `enrolled_at` khi `last_updated_at` null, giá trị lớn nhất; `deleted_at IS NULL`) có `status IN (rejected, disabled, revoked)` → `biometricReviewStatus = rejected`.
  4. Ngược lại (user không có bất kỳ row `face_profiles` hợp lệ nào) → `biometricReviewStatus = not_uploaded`.
...
BR-005: Popup chỉ hiển thị (`shouldShowBiometricPopup = true`) khi `biometricReviewStatus` — sau khi đã resolve theo đúng thứ tự ưu tiên ở BR-004 — là `not_uploaded` hoặc `rejected`. Khi `biometricReviewStatus` resolve ra `pending_review` hoặc `approved`, popup KHÔNG hiển thị, bất kể user có đồng thời các row `active`/`rejected`/`disabled`/`revoked` cũ nào khác.

BR-006: `biometricRequired = true` cho mọi `biometricReviewStatus` đã resolve (theo BR-004) khác `approved`; `biometricRequired = false` chỉ khi `biometricReviewStatus = approved`. Field này phản ánh "tổ chức yêu cầu user có biometric đã được duyệt", khác với `shouldShowBiometricPopup` (chỉ phản ánh "có nên làm gián đoạn UI bằng popup ngay bây giờ không").

BR-007: User ở trạng thái `pending_review` KHÔNG được phép submit biometric mới; API trả lỗi `BIOMETRIC_ALREADY_PENDING_REVIEW` (409).
...
BR-009 [SỬA 2026-07-29]: User ở trạng thái `approved` (đang có row `face_profiles` với `status = active`) được phép submit ảnh thay thế (replacement). Submission hợp lệ tạo một bản ghi `face_profiles` **mới** với `status = pending_review` — KHÔNG cập nhật row `active` hiện tại sang `pending_review`. Row `active` cũ giữ nguyên `status = active` trong suốt thời gian ảnh mới đang chờ duyệt. `users.avatar_url` không liên quan đến vòng đời này (feature `feat-admin-biometric-review-workflow` không còn cập nhật `users.avatar_url` khi approve — xem `feat-split-avatar-and-biometric/plan.md` quyết định D2).
...
BR-015: Backend KHÔNG lưu trạng thái "user đã dismiss popup" — việc tắt popup là hành vi tạm thời phía frontend trong phạm vi phiên làm việc hiện tại; lần đăng nhập/làm mới dữ liệu tiếp theo, nếu trạng thái vẫn là `not_uploaded`/`rejected`, hệ thống vẫn trả `shouldShowBiometricPopup = true`.

BR-REMINDER-FREQUENCY: Trong MVP, backend KHÔNG throttle tần suất nhắc biometric. Nếu `biometricReviewStatus` (đã resolve theo BR-004) là `not_uploaded` hoặc `rejected`, `shouldShowBiometricPopup` SHALL là `true` trên MỌI response login và MỌI response `GET /api/v1/me/biometric-status`, không giới hạn số lần, không có cơ chế "nhắc lại sau X ngày". Trạng thái dismiss popup chỉ tồn tại phía frontend/trong phiên làm việc hiện tại và KHÔNG được backend lưu trữ (xem BR-015).

BR-016: Login response (auth module) phải bổ sung 3 field derived (`biometricReviewStatus`, `biometricRequired`, `shouldShowBiometricPopup`) tính theo đúng BR-004/005/006, nằm cùng cấp với field `avatarUrl` hiện có trong object `user` của response login. Theo ADR-001 (Hybrid Database Access Architecture, `docs/ARCHITECTURE_DECISIONS.md`), module `auth` là ngoại lệ có chủ đích dùng raw SQL qua `DataSource.query()` và KHÔNG được refactor sang TypeORM. Do đó: auth module SHALL tính `biometricReviewStatus` bằng parameterized raw SQL (đọc trực tiếp `face_profiles`/`users` qua `DataSource.query()` với tham số binding, áp dụng đúng logic ưu tiên BR-004) ngay trong `AuthModule`, và KHÔNG được import `AccountsService`, `FaceProfileService`, hay bất kỳ TypeORM Repository nào của `AccountsModule` vào `AuthModule`. Các endpoint self-service biometric khác của feature này (`GET /api/v1/me/biometric-status`, `POST /api/v1/me/biometric-submission`) thuộc module `accounts` và được phép dùng TypeORM Repository theo convention business module thông thường (đúng ADR-001).
...
```text
FR-001: THE system SHALL xác định biometricReviewStatus của một user bằng cách áp dụng thứ tự ưu tiên resolution (pending_review > active > rejected/disabled/revoked > không có row) trên TOÀN BỘ row face_profiles chưa soft-delete của user đó, theo BR-004 — KHÔNG dựa vào một row "hiện hành" duy nhất.
FR-002 [SỬA 2026-07-29]: THE system SHALL coi users.avatar_url là dữ liệu avatar hiển thị độc lập (quản lý bởi feature `feat-update-avatar-photo`), không dùng trường này để tính biometricReviewStatus và không có liên hệ với kết quả approve biometric.
FR-003: THE system SHALL giới hạn API xem/nộp biometric trong feature này chỉ áp dụng cho chính user đã xác thực (self), không nhận tham số userId từ client.
```

### 7.2 Event-driven Requirements

```text
FR-004: WHEN user đăng nhập thành công, THE system SHALL trả về biometricReviewStatus (resolve theo thứ tự ưu tiên BR-004 trên toàn bộ row face_profiles của user), biometricRequired và shouldShowBiometricPopup trong response login theo BR-016.
FR-005: WHEN user gửi yêu cầu GET biometric-status, THE system SHALL trả về biometricReviewStatus (resolve theo thứ tự ưu tiên BR-004 trên toàn bộ row face_profiles của user), avatarUrl, biometricRequired, shouldShowBiometricPopup và message tương ứng.
FR-006: WHEN user gửi yêu cầu submit biometric hợp lệ (file đúng MIME type theo magic bytes detection, đúng kích thước, consentAccepted transform về true theo BR-011, account active) và user chưa có bất kỳ row face_profiles nào đang ở status = pending_review, THE system SHALL lưu ảnh lên Cloudinary, tạo bản ghi media_files tương ứng, tạo bản ghi face_profiles MỚI với status = pending_review và consent_at = now() trên row mới đó (theo thứ tự pre-generate UUID + insert ở mục 18.7).
FR-007: WHEN submission thành công, THE system SHALL ghi audit log với action_type biometric.upload hoặc biometric.reupload theo BR-012.
```
...
FR-009 [SỬA 2026-07-29]: WHILE user có face_profiles row với status = active (approved), THE system SHALL cho phép user submit ảnh thay thế bằng cách tạo một face_profiles row MỚI với status = pending_review; row active hiện tại giữ nguyên status cho đến khi feature feat-admin-biometric-review-workflow approve ảnh mới và revoke row cũ. `users.avatar_url` không liên quan đến quy trình này (không thay đổi trong bất kỳ trường hợp nào — xem BR-003).
FR-010: WHILE user chưa có bản ghi face_profiles hợp lệ, THE system SHALL trả biometricReviewStatus = not_uploaded và cho phép submit biometric mới.
```

### 7.4 Optional Feature Requirements

```text
FR-011: WHERE auth module hiện đọc dữ liệu user bằng raw SQL (ADR-001), THE system SHALL bổ sung phần đọc biometricReviewStatus bằng parameterized raw SQL theo đúng convention hiện có của module auth, KHÔNG import AccountsService/FaceProfileService/Repository vào AuthModule, và không refactor kiến trúc auth hiện tại (BR-016).
FR-012: WHERE media_files.storage_provider hỗ trợ giá trị cloud_provider có sẵn, THE system SHALL sử dụng giá trị này cho mọi file biometric lưu trên Cloudinary, không bổ sung giá trị enum mới cho storage_provider.
```

### 7.5 Unwanted Behavior Requirements

```text
FR-013: IF user gửi submission khi đang ở trạng thái pending_review, THEN THE system SHALL từ chối với lỗi BIOMETRIC_ALREADY_PENDING_REVIEW và không thay đổi dữ liệu.
FR-014: IF file ảnh không được gửi kèm, THEN THE system SHALL từ chối với lỗi BIOMETRIC_FILE_REQUIRED.
FR-015: IF MIME type thực tế của file — xác định bằng magic bytes detection trên buffer file, KHÔNG chỉ tin vào file.mimetype của Multer hay extension do client khai báo — không thuộc danh sách cho phép (image/jpeg, image/png, image/webp), THEN THE system SHALL từ chối với lỗi BIOMETRIC_FILE_TYPE_INVALID.
FR-016: IF file ảnh vượt quá 5MB, THEN THE system SHALL từ chối với lỗi BIOMETRIC_FILE_TOO_LARGE.
FR-017: IF consentAccepted sau khi transform theo BR-011 (chấp nhận boolean true hoặc string "true") không phải true — bao gồm false, "false", thiếu field, null, undefined, hoặc bất kỳ giá trị khác — THEN THE system SHALL từ chối với lỗi BIOMETRIC_CONSENT_REQUIRED và không lưu file.
...
FR-019: IF việc lưu ảnh lên Cloudinary thất bại, THEN THE system SHALL không tạo bản ghi media_files/face_profiles và trả lỗi BIOMETRIC_STORAGE_FAILED.
FR-020: IF có lỗi không xác định trong quá trình xử lý submission sau khi ảnh đã lưu thành công lên Cloudinary, THEN THE system SHALL trả lỗi BIOMETRIC_UPLOAD_FAILED và không để dữ liệu face_profiles ở trạng thái không nhất quán (dùng transaction).
```
...
FR-022: IF user không có permission profile.biometric.read_status, THEN THE system SHALL từ chối truy cập endpoint xem trạng thái biometric.
FR-023: IF user không có permission profile.biometric.submit, THEN THE system SHALL từ chối truy cập endpoint submit biometric và không thay đổi dữ liệu.
```
...
### 8.1 `GET /api/v1/me/biometric-status`

| Field | Value |
|---|---|
| Method | `GET` |
| Endpoint | `/api/v1/me/biometric-status` |
| Permission | `profile.biometric.read_status` |
...
```json
{
  ...
  "data": {
    ...
    "avatarUrl": "https://res.cloudinary.com/.../biometric.jpg",
...
### 8.2 `POST /api/v1/me/biometric-submission`

| Field | Value |
|---|---|
| Method | `POST` |
| Endpoint | `/api/v1/me/biometric-submission` |
| Content-Type | `multipart/form-data` |
| Permission | `profile.biometric.submit` |
| System Role | `INTERNAL_USER`, `MANAGER`, `BUSINESS_ADMIN`, `SYSTEM_ADMIN` |
| Auth | JWT access token (self, không nhận `userId` param) |
| Async | No |
...
```json
{
  ...
  "error": {
    "code": "BIOMETRIC_ALREADY_PENDING_REVIEW",
    "details": {}
  },
...
| Consent | Vì request là `multipart/form-data`, backend SHALL transform `consentAccepted` trước khi validate: chấp nhận boolean `true` hoặc string `"true"`; mọi giá trị khác (`false`, `"false"`, thiếu field, `null`, `undefined`, `"1"`, `"yes"`, ...) đều bị reject với `BIOMETRIC_CONSENT_REQUIRED` (BR-011) |
| Account status | `users.account_status = active` mới được submit; không áp dụng cho GET biometric-status |
...
|---|---|---|---|
| `profile.biometric.read_status` | `accounts` | Xem trạng thái biometric/face profile của chính mình | `INTERNAL_USER`, `MANAGER`, `BUSINESS_ADMIN`, `SYSTEM_ADMIN` |
| `profile.biometric.submit` | `accounts` | Tự nộp/nộp lại biometric của chính mình | `INTERNAL_USER`, `MANAGER`, `BUSINESS_ADMIN`, `SYSTEM_ADMIN` |
...
|---:|---|---|
| 400 | `BIOMETRIC_FILE_REQUIRED` | Thiếu file ảnh trong request |
| 400 | `BIOMETRIC_FILE_TOO_LARGE` | File vượt quá 5MB |
| 400 | `BIOMETRIC_FILE_TYPE_INVALID` | MIME type không hợp lệ (xác định bằng magic bytes detection, không chỉ dựa vào mimetype/extension client khai báo) |
| 400 | `BIOMETRIC_CONSENT_REQUIRED` | `consentAccepted` không phải `true` |
...
| 409 | `BIOMETRIC_ALREADY_PENDING_REVIEW` | Đang có submission chờ duyệt |
| 502 | `BIOMETRIC_STORAGE_FAILED` | Lưu ảnh lên Cloudinary thất bại |
| 500 | `BIOMETRIC_UPLOAD_FAILED` | Lỗi xử lý submission sau khi đã lưu ảnh thành công |
...
```text
ERR-001: IF request submit thiếu file, THEN THE system SHALL trả 400 BIOMETRIC_FILE_REQUIRED.
ERR-002: IF file vượt quá 5MB, THEN THE system SHALL trả 400 BIOMETRIC_FILE_TOO_LARGE.
ERR-003: IF MIME type không thuộc jpg/jpeg/png/webp, THEN THE system SHALL trả 400 BIOMETRIC_FILE_TYPE_INVALID.
ERR-004: IF consentAccepted không phải true, THEN THE system SHALL trả 400 BIOMETRIC_CONSENT_REQUIRED.
...
ERR-008: IF face_profiles.status hiện tại là pending_review khi submit, THEN THE system SHALL trả 409 BIOMETRIC_ALREADY_PENDING_REVIEW.
ERR-009: IF lưu file lên Cloudinary thất bại, THEN THE system SHALL trả 502 BIOMETRIC_STORAGE_FAILED và không tạo media_files/face_profiles.
ERR-010: IF có lỗi server không xác định sau khi ảnh đã lưu thành công, THEN THE system SHALL trả 500 BIOMETRIC_UPLOAD_FAILED và rollback toàn bộ thay đổi face_profiles/media_files của lần submit đó.
```

### 11.2 Validation / Error Precedence

Để đảm bảo các test case có nhiều lỗi xảy ra đồng thời vẫn cho kết quả deterministic, `POST /api/v1/me/biometric-submission` PHẢI kiểm tra theo đúng thứ tự sau, dừng ngay tại bước đầu tiên phát hiện lỗi:

1. Auth guard → 401 `UNAUTHORIZED` nếu chưa đăng nhập / token không hợp lệ.
2. Permission guard → 403 `FORBIDDEN` nếu thiếu permission `profile.biometric.submit`.
...
5. Multipart/file presence → 400 `BIOMETRIC_FILE_REQUIRED` nếu thiếu field `file`.
6. File size → 400 `BIOMETRIC_FILE_TOO_LARGE` nếu vượt quá 5MB.
7. Magic bytes MIME validation → 400 `BIOMETRIC_FILE_TYPE_INVALID` nếu không phải image/jpeg, image/png, image/webp (mục 9, VL-02).
8. `consentAccepted` transform + validate → 400 `BIOMETRIC_CONSENT_REQUIRED` nếu không hợp lệ sau transform (BR-011, VL-01).
9. Pending-review check → 409 `BIOMETRIC_ALREADY_PENDING_REVIEW` nếu user đã có row `face_profiles.status = pending_review` (BR-007, BR-010).
10. Storage upload (Cloudinary) / DB transaction → theo đúng thứ tự pre-generate UUID + insert ở mục 18.7; lỗi ở bước này trả `BIOMETRIC_STORAGE_FAILED` (502) hoặc `BIOMETRIC_UPLOAD_FAILED` (500) tùy giai đoạn thất bại, kèm best-effort cleanup theo EC-004.

`GET /api/v1/me/biometric-status` chỉ áp dụng bước 1–3 ở trên (không có bước 4–10 vì là hành vi chỉ đọc, không ghi dữ liệu — xem EC-006).
...
|---|---|---|---|
| `biometric.upload` | `face_profile` | Submission thành công lần đầu (chưa từng có `face_profiles` row) | `user_id` (actor = chính user), `entity_id = face_profiles.id`, `new_value_json = { status: 'pending_review', mediaFileId }` |
| `biometric.reupload` | `face_profile` | Submission thành công khi đã có `face_profiles` row từ trước (rejected/disabled/revoked/active → pending_review) | `user_id`, `entity_id = face_profiles.id`, `old_value_json = { status: <trạng thái trước> }`, `new_value_json = { status: 'pending_review', mediaFileId }` |

Quy tắc:
- Không ghi nội dung binary file vào audit log.
- Không ghi audit log cho hành vi GET biometric-status (chỉ đọc, không phải hành vi ghi/security/admin/export theo quy ước audit của `AGENTS.md` §17).
- Audit log ghi sau khi transaction submission commit thành công; nếu submission thất bại thì không ghi `biometric.upload`/`biometric.reupload` (có thể ghi log lỗi hệ thống riêng nếu cần, không thuộc audit nghiệp vụ).
...
- Reminder ở feature này là **derived data hiển thị trong response** (login response + GET biometric-status), không phải notification asynchronous.
- Notification khi biometric bị reject thuộc trách nhiệm của `feat-admin-biometric-review-workflow` (do hành động reject xảy ra ở feature đó).
...
When user đăng nhập,
Then response login trả biometricReviewStatus = "not_uploaded", biometricRequired = true, shouldShowBiometricPopup = true.

AC-002:
Given user ở trạng thái not_uploaded,
When user gọi POST /api/v1/me/biometric-submission với file hợp lệ và consentAccepted = true,
Then the system trả 201, tạo media_files mới, tạo face_profiles với status = pending_review, không cập nhật users.avatar_url, và ghi audit log action_type = biometric.upload.
...
When user gọi GET /api/v1/me/biometric-status,
Then the system áp dụng thứ tự ưu tiên BR-004 và trả biometricReviewStatus = "pending_review" (KHÔNG phải "approved"), shouldShowBiometricPopup = false, biometricRequired = true.

AC-003b:
Given user chỉ có 1 row face_profiles với status = pending_review (không có row active/rejected/disabled/revoked nào khác),
When user gọi GET /api/v1/me/biometric-status,
Then the system trả biometricReviewStatus = "pending_review", shouldShowBiometricPopup = false, biometricRequired = true.

AC-004:
Given user đang ở trạng thái rejected,
When user đăng nhập,
Then the system trả biometricReviewStatus = "rejected", shouldShowBiometricPopup = true.

AC-005:
Given user đang ở trạng thái rejected (có row face_profiles với status = rejected),
When user gọi POST /api/v1/me/biometric-submission với file hợp lệ,
Then the system tạo một row face_profiles MỚI với status = pending_review, trả 201, và ghi audit log action_type = biometric.reupload.
  And row face_profiles cũ (status = rejected) giữ nguyên, không bị cập nhật.

AC-006 [SỬA 2026-07-29]:
Given user đang ở trạng thái approved (có row face_profiles với status = active),
When user đăng nhập,
Then the system trả biometricReviewStatus = "approved", biometricRequired = false, shouldShowBiometricPopup = false.
  And `avatarUrl` phản ánh đúng `users.avatar_url` hiện tại, có thể null hoặc không-null tùy user đã tự upload avatar qua feature `feat-update-avatar-photo` hay chưa — KHÔNG có quan hệ nhân quả với việc biometric vừa được approved (bản cũ mô tả sai quan hệ này, xem `feat-split-avatar-and-biometric/plan.md` quyết định D2).

AC-006b:
Given user đang ở trạng thái approved (có row face_profiles với status = active),
When user gọi POST /api/v1/me/biometric-submission với file hợp lệ (ảnh thay thế),
Then the system tạo một row face_profiles MỚI với status = pending_review, trả 201, và ghi audit log action_type = biometric.reupload.
  And row face_profiles cũ (status = active) giữ nguyên, users.avatar_url không thay đổi.
  And khi user gọi tiếp GET /api/v1/me/biometric-status ngay sau đó, the system trả biometricReviewStatus = "pending_review" theo thứ tự ưu tiên BR-004 (xem AC-003), KHÔNG trả "approved".
```
...
Then the system trả 400 BIOMETRIC_FILE_REQUIRED.
...
When request được xử lý,
Then the system phát hiện qua magic bytes detection (không tin vào extension/Content-Type khai báo) và trả 400 BIOMETRIC_FILE_TYPE_INVALID.

AC-009:
Given user gửi file 8MB,
When request được xử lý,
Then the system trả 400 BIOMETRIC_FILE_TOO_LARGE.

AC-010:
Given user gửi consentAccepted = false (hoặc string "false", hoặc thiếu field consentAccepted),
When request được xử lý,
Then the system trả 400 BIOMETRIC_CONSENT_REQUIRED và không lưu file lên Cloudinary.
...
When gọi GET /api/v1/me/biometric-status hoặc POST /api/v1/me/biometric-submission,
Then the system trả 401 UNAUTHORIZED.

AC-012:
Given user đã đăng nhập nhưng không có permission profile.biometric.submit,
When user gọi POST /api/v1/me/biometric-submission,
...
When user gọi POST /api/v1/me/biometric-submission,
Then the system trả 409 BIOMETRIC_ALREADY_PENDING_REVIEW và không thay đổi face_profiles/media_files hiện có.
...
Then không có bảng database mới nào được tạo,
  ...
  And có 1 seed/migration idempotent tạo 2 permission profile.biometric.read_status và profile.biometric.submit, gán cho INTERNAL_USER, MANAGER, BUSINESS_ADMIN, SYSTEM_ADMIN qua role_permissions theo pattern src/database/seeds/*SeedXxxPermission*.ts hiện có (AR-01),
...
When user gọi GET /api/v1/me/biometric-status,
Then the system trả biometricReviewStatus = "rejected",
  And shouldShowBiometricPopup = true,
...
EC-006: User gọi `GET /api/v1/me/biometric-status` khi `account_status` không phải `active` (ví dụ `locked`) → vẫn cho phép xem trạng thái (read-only, không rủi ro ghi dữ liệu); giới hạn `ACCOUNT_NOT_ACTIVE` chỉ áp dụng cho hành vi submit (ghi dữ liệu).
...
```text
NFR-001: THE system SHALL phản hồi GET /api/v1/me/biometric-status trong vòng 1 giây ở điều kiện tải bình thường.
NFR-002: THE system SHALL phản hồi POST /api/v1/me/biometric-submission trong vòng 5 giây ở điều kiện tải bình thường, không tính thời gian network upload file của client.
```
...
NFR-010: THE system SHALL log lỗi BIOMETRIC_STORAGE_FAILED và BIOMETRIC_UPLOAD_FAILED với đủ thông tin để debug (không bao gồm nội dung file); khi xảy ra BIOMETRIC_UPLOAD_FAILED sau khi ảnh đã lưu thành công lên Cloudinary, THE system SHALL log kết quả của thao tác best-effort cleanup Cloudinary (thành công ở mức info, thất bại ở mức warning/error) kèm public_id và request_id để hỗ trợ dọn dẹp/tra cứu thủ công sau (EH-01, EC-004).
```
...
- **`feat-admin-biometric-review-workflow` (feature song song, agent khác phụ trách)**: là nơi duy nhất được phép chuyển `face_profiles.status` từ `pending_review` sang `active`/`rejected`. [SỬA 2026-07-29] Feature đó KHÔNG còn cập nhật `users.avatar_url` khi approve (đã bỏ theo `feat-split-avatar-and-biometric/plan.md` quyết định D2) — `users.avatar_url` do feature độc lập `feat-update-avatar-photo` quản lý. Feature này phụ thuộc vào kết quả của feature đó để vòng đời trạng thái hoàn chỉnh (not_uploaded → pending_review → approved/rejected), nhưng không triển khai phần đó. **Approval actor của feature đó là `SYSTEM_ADMIN` hoặc `MANAGER`** (đã xác nhận khớp với `feat-admin-biometric-review-workflow/spec.md` hiện tại, DRIFT-FIX mục 2.2 — không còn giới hạn chỉ `SYSTEM_ADMIN` như bản cũ). Feature này KHÔNG mở rộng quyền approve/reject cho actor nào khác.
...
- **RBAC / `permissions` table**: cần thêm 2 permission mới (`profile.biometric.read_status`, `profile.biometric.submit`) gán cho 4 system role hiện có. Nguồn triển khai chính thức SHALL là TypeORM seed/migration idempotent theo pattern `src/database/seeds/*SeedXxxPermission*.ts` (mục 10, AR-01) — không thay đổi cấu trúc RBAC.
- **Partial unique index `ux_face_profiles_user_pending`**: cả feature này và `feat-admin-biometric-review-workflow` đều tham chiếu cùng 1 index. Ghi chú đối chiếu: tại thời điểm viết spec này, `feat-admin-biometric-review-workflow/spec.md` (mục 10.4) vẫn ghi index này là "đề xuất"; spec hiện tại (DM-01) đã chốt index này là BẮT BUỘC. Khuyến nghị team đồng bộ wording ở cả 2 spec khi có dịp cập nhật, để tránh hiểu nhầm rằng index là tùy chọn.

---

## 18. Data Model Impact

> Không thêm bảng mới. Có 2 thay đổi schema/data nhỏ BẮT BUỘC (không còn là đề xuất tùy chọn): (1) migration DDL tạo partial unique index `ux_face_profiles_user_pending` (DM-01, mục 18.5); (2) seed/migration idempotent tạo 2 permission `profile.biometric.read_status`/`profile.biometric.submit` (AR-01, mục 10). Ngoài 2 thay đổi nhỏ này, phần còn lại (bổ sung giá trị `rejected` cho enum `FaceProfileStatus`) chỉ ở mức app-level enum, không cần migration DDL riêng.
...
| `face_profiles` | Source of truth cho trạng thái biometric/face candidate | Dùng lại `FaceProfileEntity`/`FaceProfileStatus` hiện có; cần bổ sung giá trị enum `rejected` |
...
| `audit_logs` | Ghi nhận `biometric.upload`/`biometric.reupload` | Dùng lại `AuditLogEntity` hiện có |
| `permissions`, `role_permissions` | Thêm 2 permission mới cho self-service biometric | Theo cơ chế seed hiện tại, không đổi schema |
...
- Mỗi user chỉ có tối đa 1 row `face_profiles` có `status = active` tại một thời điểm (đảm bảo bởi feat-admin-biometric-review-workflow khi approve).
...
FR-DATA-003: IF user đã có bất kỳ row face_profiles nào với status = pending_review, THEN THE system SHALL từ chối submission với lỗi BIOMETRIC_ALREADY_PENDING_REVIEW (BR-007).
...
10. INSERT audit_logs với action_type = biometric.upload hoặc biometric.reupload (BR-012), entity_id = faceProfileId.
11. COMMIT.
12. Nếu transaction DB thất bại sau khi Cloudinary upload đã thành công (bước 5) → thực hiện best-effort cleanup Cloudinary object theo EH-01/EC-004, sau đó trả lỗi BIOMETRIC_UPLOAD_FAILED.
```
...
- Admin approve/reject biometric — đã có/sẽ có ở `feat-admin-biometric-review-workflow`.
- Notification (in-app/email) khi biometric bị reject hoặc được approve.
...
- Chặn user sử dụng các tính năng khác của hệ thống khi chưa có biometric hoặc biometric bị reject.
- Thay đổi RBAC ngoài việc thêm 2 permission `profile.biometric.read_status`/`profile.biometric.submit`.
...
- Push realtime (WebSocket) khi biometric được approve, để cập nhật `shouldShowBiometricPopup` không cần đăng nhập lại.
...
OOS-005: THE system SHALL NOT block access to any other system feature based on biometric status.
...
- **Mapping `disabled`/`revoked` → response `rejected`** (BR-004): đã quyết định để tránh mở rộng enum response ngoài 4 giá trị mà yêu cầu gốc đã chốt (`not_uploaded`, `pending_review`, `rejected`, `approved`). Nếu `feat-admin-biometric-review-workflow` cần phân biệt rõ message hiển thị giữa "bị reject" và "bị disabled/revoked", có thể cần bổ sung field phụ (ví dụ `avatarReviewReason`) ở một spec sau — KHÔNG thêm trong spec này để tránh mở rộng scope.
- **Submit lại khi đang `approved` (BR-009)**: đã quyết định cho phép, tạo row `face_profiles` MỚI với `status = pending_review`. Row `active` cũ giữ nguyên trong thời gian ảnh mới đang chờ duyệt. Khi admin approve ảnh mới, `feat-admin-biometric-review-workflow` sẽ revoke old active row. Admin có thể nhận diện đây là ảnh thay thế vì cùng `user_id` đang có cả row `active` lẫn row `pending_review` cùng tồn tại. [SỬA 2026-07-29] `users.avatar_url` không liên quan đến quy trình này trong bất kỳ giai đoạn nào (xem BR-009, BR-003).
...
| BL-03 | Biometric status resolution priority khi user có nhiều row | BR-004, BR-005, BR-006, FR-001, FR-004, FR-005, AC-003, AC-003b, AC-006b, EC-007 |
| AR-01 | Permission seed qua migration/seed idempotent | Mục 10, mục 18.5, AC-015 |
| AR-02 [SỬA 2026-07-29] | SYSTEM_ADMIN hoặc MANAGER approve/reject biometric (feature khác — xem DRIFT-FIX tại `feat-admin-biometric-review-workflow/spec.md` mục 2.2) | Mục 17 (Dependencies) |
...
- [x] Đã thêm biometric status resolution priority (BR-004) áp dụng trên toàn bộ row face_profiles, không còn mâu thuẫn active-cũ/pending-mới (BL-03).
