...
- **BUG-01 [ĐÍNH CHÍNH 2026-07-29 — KHÔNG PHẢI BUG THẬT]**: Ban đầu nghi ngờ `face-provisioning.service.ts` gọi `getPortraitBytes(userId)` không kiểm tra `face_profiles.status === 'active'`. Đã kiểm tra `git log -S "R2 + VAL-01"` và xác nhận **SAI** — `getPortraitBytes` đã được vá lọc `status = 'active'` từ TRƯỚC, tại commit `b2c34ce` ("fix(accounts): getPortraitBytes lấy ảnh ACTIVE từ Cloudinary cho IVSS enroll (FPB-001)", 2026-06-30) — tức là code thật đã đúng, KHÔNG cần sửa gì ở `face-provisioning.service.ts`/`face-profile.service.ts`. Việc còn lại chỉ là cập nhật spec `feat-meeting-face-provisioning`/`feat-portrait-enrollment` cho đúng thực tế (đã làm ở T10/T11, sau đó đính chính lại lần nữa khi phát hiện sai sót này — xem changelog 2 file đó).
