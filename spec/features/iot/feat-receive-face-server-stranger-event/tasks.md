# Tasks: Receive Stranger Event from Face Server (IOT-008)

## 1. Kiểm tra codebase hiện tại
- [x] Kiểm tra `src/modules/iot/iot.module.ts` để biết cách đăng ký controller.
- [x] Kiểm tra `src/modules/iot/controllers/device-callbacks.controller.ts` đã có heartbeat/verify endpoints chưa.
- [x] Kiểm tra `src/modules/iot/controllers/verify-short-device-callbacks.controller.ts` để tham khảo route short alias `vf`.
- [x] Kiểm tra `src/modules/iot/controllers/short-device-callbacks.controller.ts` để tránh nhầm với route `hb`.
- [x] Kiểm tra `src/modules/iot/services/iot-devices.service.ts` và các helper từ IOT-006/IOT-007 như extract value, verify token, normalize IP, build payload sample.
...
## 2. Canonical stranger endpoints
- [x] Cập nhật `src/modules/iot/controllers/device-callbacks.controller.ts`.
- [x] Thêm endpoint `GET /api/v1/device-callbacks/face/stranger`.
- [x] Thêm endpoint `POST /api/v1/device-callbacks/face/stranger`.
- [x] GET và POST phải gọi chung service method `IotDevicesService.receiveStrangerEvent(...)`.
...
## 3. Short alias stranger controller
- [x] Tạo file `src/modules/iot/controllers/stranger-short-device-callbacks.controller.ts`.
- [x] Đặt controller prefix `@Controller('sf')`.
- [x] Thêm endpoint `GET /api/v1/sf/:deviceCode/:callbackToken`.
- [x] Thêm endpoint `POST /api/v1/sf/:deviceCode/:callbackToken`.
- [x] Cả GET và POST gọi chung `IotDevicesService.receiveStrangerEvent(...)`.
...
- [x] Đảm bảo route thật là `/api/v1/sf/:deviceCode/:callbackToken`, không phải `/api/v1/hb/sf/...` hoặc `/api/v1/vf/sf/...`.

## 4. Đăng ký controller trong module
- [x] Cập nhật `src/modules/iot/iot.module.ts`.
- [x] Thêm `StrangerShortDeviceCallbacksController` vào `controllers`.
- [x] Đảm bảo `DeviceCallbacksController` vẫn hoạt động cho heartbeat/verify/stranger canonical endpoints.
- [x] Đảm bảo không ảnh hưởng Admin API controller.

## 5. Service method
- [x] Cập nhật `src/modules/iot/services/iot-devices.service.ts`.
- [x] Tạo internal input interface/type cho stranger event request context.
- [x] Tạo method `receiveStrangerEvent(input)`.
- [x] Method xử lý chung cho GET/POST/canonical/alias.
- [x] Tái sử dụng helper từ IOT-006/IOT-007 nếu có, tránh copy-paste lệch logic.
- [x] Không xử lý verify/attendance/check-in/check-out/presence/no-show/alert/recording trong method này.
...
- [x] Nếu không có `callback_token_hash`, throw `ConflictException` với code `CALLBACK_TOKEN_NOT_CONFIGURED`.
...
- [x] Reuse/helper logic từ IOT-006/IOT-007 nếu có.
...
- [x] Nếu file vượt quá 5MB hoặc quá 5 files, trả `413 Payload Too Large`.

## 12. Build stranger payload sample
- [x] Tạo `raw_payload_sample` bằng cách gộp body text fields, file metadata và request metadata.
...
## 13. DB update
- [x] Khi stranger event hợp lệ, update:
  - `lastSeenAt = now`
  - `status = online`
  - `healthStatus = healthy`
- [x] Clone/merge `metadataJson` cũ.
- [x] Update `metadataJson.last_stranger_event_sample = newSample`.
- [x] Update `metadataJson.recent_stranger_event_samples = [newSample, ...oldSamples].slice(0, 5)`.
...
- [x] Không xóa metadata cũ:
  ...
  - `last_verify_event_sample`
  - `recent_verify_event_samples`
  - `rtsp_config`
  - `vendor`
  - `connection`
...
- [x] Không tạo notification/security alert.
...
- [x] Test GET `/api/v1/sf/:deviceCode/:callbackToken` thành công.
- [x] Test POST `/api/v1/sf/:deviceCode/:callbackToken` thành công.
...
- [x] Test file vượt 5MB → `413 Payload Too Large`.
...
- [x] Test missing callback token hash → `ConflictException` / `CALLBACK_TOKEN_NOT_CONFIGURED`.
...
- [x] Test `last_stranger_event_sample` được cập nhật.
- [x] Test `recent_stranger_event_samples` tối đa 5 item.
...
- [x] Test không tạo notification/security alert.
...
- [x] Test bằng curl multipart giả lập Snap:
  `curl -X POST "http://localhost:3000/api/v1/sf/TEST-CAM-001/<token>" -F "person_id=123" -F "image=@/path/to/dummy.jpg"`
- [x] Cấu hình Face Server thật:
  - `Stranger Subscription = Subscription Snap`
  - `Snap URL = /api/v1/sf/TEST-CAM-001/<short_token>`
- [x] Cho người lạ hoặc mặt chưa đăng ký đi qua camera.
- [x] Kiểm tra PostgreSQL:
  - `metadata_json->'last_stranger_event_sample'`
  - `metadata_json->'recent_stranger_event_samples'`
- [x] Xác nhận `recent_stranger_event_samples` lưu đúng sample.
- [x] Xác nhận không có notification/security alert được tạo.
